...
def sample_path(mu_enter, mu_exit, lambd, grid_size, t_interval, T, loc_set, num_drivers_enter, num_orders_placed):
    # select sample path 
    #Determines a realization of order and driver arrivals  
    m_n = {}
    d_n = {}
    for t in range(T):
...
# Verified for 1. disagg alg, 2.
class Driver:
    def __init__(self, number, sa, ma, loc, loc_1, loc_2, time_entered, time_first_matched, ha, bar_ha, profit_matching):
        self.number = number
...
def fetch_vf(aggregate, key_0, v_t, driver_attr_a):
    if not aggregate:
        if key_0 in v_t['last_met']:
            n_last = v_t['last_met'][key_0]
            vf = v_t[n_last][key_0]
        else:
            vf = 0
    else:
        key_1, key_2 = list(copy.copy(key_0)), list(copy.copy(key_0))
        key_1[5], key_2[5] = tuple(driver_attr_a.loc_1), tuple(driver_attr_a.loc_2)
        key_1, key_2 = tuple(key_1), tuple(key_2)
        vf_list = []
        for key in [key_0, key_1, key_2]:
            if key in v_t['last_met']:
                n_last = v_t['last_met'][key]
                vf_list.append(v_t[n_last][key])
            else:
                vf_list.append(0)
        w = compute_weights(key_0, key_1, key_2, v_t)
        vf = np.dot(w, vf_list)
    return vf
        ...
        for b in demand_list['active']:
            ...
            if time <= demand_attr[b].delivery_deadline:  # if can be delivered before deadline
                ...
                if g > 0 or (g <= 0 and bar_ha_new >= data['W']):  # if there's time or no time but guarantee met
                    c = theta + phi - 0.5*driver_pay_ab + cost_to_go + data['alpha_penalty'] * (rho_a + rho_b)
...
def update_attr(driver_list, demand_list, xab, driver_attr, demand_attr, t, data, x_ab_time, x_ab_cost, aggregate, stats=False,
            ...
            if driver_attr[a].sa == 0:  # if driver is inactive  # driver attr does not change
                ...
                if ((t+1)*data['t_interval'] - driver_attr[a].time_first_matched) >= data['Guaranteed_service']:
                    ...
                    if stats:
                        ...
                        if bar_ha >= data['W']:
                            if 'guarantee_met' not in driver_stats:
                                driver_stats['guarantee_met'] = []
                            driver_stats['guarantee_met'].append([a, bar_ha, [profit_matching],
                                                                  driver_attr[a].active_history])
                        else:
                            if 'guarantee_missed' not in driver_stats:
                                driver_stats['guarantee_missed'] = []
                            driver_stats['guarantee_missed'].append([a, bar_ha, [profit_matching],
                                                                  driver_attr[a].active_history])

                    driver_attr[a] = Driver(a, 0, 1, loc_d, loc_1, loc_2, driver_attr[a].time_entered, 0, 0, 0, 0)
                    driver_temp['inact'].append(a)
            ...
            for a in driver_list['act_unavailable']:
                ...
                if avail_time <= (t+1)*data['t_interval']:  # if driver will be avail at t+1
                    if ((t+1)*data['t_interval'] - driver_attr[a].time_first_matched) >= data['Guaranteed_service']:
                        ...
                        driver_attr[a] = Driver(a, 0, 1, loc, loc_1, loc_2, driver_attr[a].time_entered, 0, 0, 0, 0)
    ...
    #print('driver_stats: ', driver_stats); #driver_stats['guarantee_met']
    #print() 

    return driver_temp, demand_temp, driver_stats, demand_stats 
    ...
    def update_driver_attr(num_drivers):
            ...
            if (t+1)*data["t_interval"] - driver_attr[a].time_first_matched >= data['Guaranteed_service']:
                ...
                if stats:
                    driver_log[a]['all_t'].append(['t%d' % (t + 1), [driver_attr[a].sa, driver_attr[a].ma,
                     driver_attr[a].loc, driver_attr[a].time_first_matched, driver_attr[a].time_entered, driver_attr[a].ha, driver_attr[a].bar_ha]])
        ...
        if stats and t+1 >= data['T']:
            ...
            for j in range(m_n[t+1]):  # create new driver objects range(m[n][t+1])
                ...
                driver_attr[a] = Driver(a, 0, 1, loc, loc_1, loc_2, t+1, 0, 0, 0, 0)
