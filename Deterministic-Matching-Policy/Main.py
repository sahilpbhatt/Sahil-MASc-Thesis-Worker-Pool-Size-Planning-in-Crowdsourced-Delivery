...
def iterate(data, d, penalty, cfa, pool_sizes, V, alpha):
    # Step 3.1: while n <= N
    ...
    for n in range(1, N):
        ...
        for t in range(data['T']):
            ...
            xab, z, dual_a, num_matches, x_ab_time, x_ab_cost, dst, tot_dst, platform_profit = func.solve_opt(data, t, n, demand_list, driver_list,
                                                            driver_attr, demand_attr, penalty, cfa)
            obj_val[n] += z
    ...
    confidence_interval(overall_fraction_demand_met); 
...
def run(penalty, data, data_set, cfa=False): 
    ...
    #pool_sizes = [8, 8, 1, 2, 4, 2, 5, 4, 2, 4, 6, 1, 1, 1, 6, 1] 
    #pool_sizes = [10 for i in range(16)]; 
    
    print('pool_sizes are: ', data['pool_sizes'])
    ...
    for i in range(I):
        ...
        print('pool_sizes: ', data['pool_sizes'])

        obj_val_list, time_algorithm, av_fraction_demand_met, av_fraction_meeting_guarantee, V, alpha, buckets = iterate(data, d, penalty, cfa, data['pool_sizes'], V, alpha)
        ...
        for p in range(16):
            ...
            alpha[p, data['pool_sizes'][p]] += 1

            if alpha[p, data['pool_sizes'][p]] == 0:
                alpha_step_size = 1
            else: 
                alpha_step_size = 1/(alpha[p, data['pool_sizes'][p]])**0.5 
            ...
            V[p, data['pool_sizes'][p]] = (1-alpha_step_size)*V[p, data['pool_sizes'][p]] + alpha_step_size*V_new
...
if __name__ == '__main__':
    ...
    weight_driver_utilization = 0; 
    pool_sizes = [23, 22, 37, 23, 45, 19, 45, 46, 48, 23, 48, 14, 34, 31, 33, 42]

    data = {'T': 192, 'pool_sizes':pool_sizes, 'train':False,'train_I':250, 'train_N':11, 'test_I':1, 'test_N':11, 
