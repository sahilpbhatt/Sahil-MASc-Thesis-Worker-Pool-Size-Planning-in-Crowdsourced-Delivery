...
# Setup model
def setup_model_cfa(data, t, n, demand_list, driver_list, driver_attr, demand_attr, penalty):
    ...
    for a in drivers:
        ...
        g = max(driver_attr[a].exit_time*data['t_interval'] - ((t + 1) * data['t_interval'] - driver_attr[a].time_entered), 0)
        ...
        for b in demand_list['active']:
            ...
            if time <= min(demand_attr[b].delivery_deadline, driver_attr[a].exit_time*data['t_interval']):  # if can be delivered before deadline
                ...
                g = max(driver_attr[a].exit_time*data['t_interval'] - (time - driver_attr[a].time_entered), 0)
                ...
                x_ab_cost[(a, b)] = c #- data['w_s']*(theta + phi - 0.5*driver_pay_ab)
                x_ab_time[(a, b)] = time
...
def solve_opt(data, t, n, demand_list, driver_list, driver_attr, demand_attr, penalty, cfa):
